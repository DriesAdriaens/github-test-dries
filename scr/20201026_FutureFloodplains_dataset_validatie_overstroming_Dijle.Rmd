...
```{r}
meetpunten_sel <- komgr_neerijse_loc %>% filter(filterdepth < 3) %>% collect()
```
...
```{r}
meetpunten_sel <- rbind(meetpunten_sel, komgr_doodebeemde_loc %>% filter(filterdepth <= 2.5) %>% collect())
```
...
```{r}
meetpunten_sel <- rbind(meetpunten_sel, komgr_langerode_loc %>% filter(filterdepth <= 3) %>% collect())
```
...
Uit bovenstaande screening besluiten we dat er in de komgrond van Langerode geen meetpunten zijn die metingen bevatten die bruikbaar zijn om gemodelleerde overstromingsdiepten te kalibreren.
...
```{r}
meetpunten_sel <- rbind(meetpunten_sel, komgr_leibeekO_loc %>% filter(filterdepth <= 2.5) %>% collect())
```
...
```{r}
meetpunten_sel <- rbind(meetpunten_sel, komgr_leigrachtO_loc %>% filter(filterdepth <= 2.5) %>% collect())
```
...
leaflet(meetpunten_sel_def %>% 
  ...
  addCircleMarkers(
    data = get_locs(watina, area_codes = "DYL",
      ...
      st_transform(crs = 4326),
    label = ~loc_code, 
    fill = FALSE, weight = 3, color = "grey", opacity = 0.5,
...
Vraag of de dataset enkel de metingen boven maaiveld moet bevatten, dan wel alle beschikbare metingen, voor de geselecteerde meetpunten.
