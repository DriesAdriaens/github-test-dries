...
Noot: het is onduidelijk of ook de meetpunten ten oosten van de spoorwegbedding tot deze komgrond moeten gerekend worden. In principe lijkt dit stuk wel hydrologisch verbonden via de doorgang onder de spoorweg (verbindingsweggetje naar Waversebaan).
...
```{r}
komgr_doodebeemde_loc_vec <- c(
  # meetpunten ten oosten van spoorweg; mee te nemen?
  # "DYLP052", "DYLB050", "DYLB051", "DYLB056", "DYLP053", "DYLP056", "DYLP012", "DYLP013", "DYLP026", "DYLP038", "DYLP050", "DYLP051", "DYLP037",
  "BAOL502", "DYLR003", "DYLP075", "DYLP041", "DYLB039", "DYLB093", "DYLP024", "DYLP040", "DYLP007", "DYLP008", "DYLP009", "DYLP015", "DYLP017", "DYLP022", "DYLP039", "DYLP054", "DYLP059", "DYLP062", "DYLP063", "DYLP074", "DYLP091", "DYLP092", "DYLP093", "DYLS003", "DYLS004", "DYLP016", "DYLP004", "DYLP006", "DYLP042", "DYLP055", "DYLP023")
```
...
Uit de kaart en bovenstaande figuur leiden we af dat er 5 diepere (> 2.5 m diep) peilbuizen zijn, waarvan er 4 deel uitmaken van een peilbuiskoppel (ondiep-diep). Eén peilbuiskoppel bestaat uit 2 diepere peilbuizen.
...
komgr_doodebeemde_tijdreeks_plot <- ggplot(komgr_doodebeemde_tijdreeks) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
                colour = loc_code)) +
  ## non-clipping zoom
...
komgr_doodebeemde_tijdreeks_plot
```
...
```{r boven maaiveld doodebeemde}
aantal_doodebeemde <- komgr_doodebeemde_tijdreeks %>% 
  filter(mMaaiveld >= 0) %>% 
  select(loc_code) %>% 
  distinct() %>% 
  count() %>% 
...
In `r aantal_doodebeemde` van de `r length(komgr_doodebeemde_loc_vec)` meetpunten in de komgrond van de Doode Beemde hebben de stijghoogten dus ooit eens boven maaiveld gelegen.

Figuur met de tijdreeksen voor de subset van meetpunten met minstens één of meerdere stijghoogten boven maaiveld:

```{r tijdreeks_doodebeemde_plot_bovenmaaiveld, message=FALSE, warning=FALSE}
...
Tijdreeksen voor meetpunten met gemeten stijghoogten hoger dan 25 cm boven maaiveld, in m-mv:

```{r tijdreeks_doodebeemde_plot_bovenmaaiveld25cm_mv, message=FALSE, warning=FALSE}
temp <- komgr_doodebeemde_tijdreeks %>% 
                  filter(mMaaiveld > 0.25) %>% 
                  select(loc_code) %>% 
                  distinct() %>% 
                  pull()
ggplot(komgr_doodebeemde_tijdreeks %>% 
...
in m-mv, voor peilbuizen tot en met DYLP016, met vermelding van filterdiepte:
```{r tijdreeks_doodebeemde_plot_bovenmaaiveld25cm_mv1, message=FALSE, warning=FALSE}
temp <- komgr_doodebeemde_tijdreeks %>% 
                  filter(mMaaiveld > 0.25 & as.numeric(str_sub(loc_code,5,7)) <= 16) %>% 
                  select(loc_code) %>% 
                  distinct() %>% 
                  pull()
ggplot(komgr_doodebeemde_tijdreeks %>% 
...
in m-mv, voor peilbuizen vanaf DYLP016, met vermelding van filterdiepte:
```{r tijdreeks_doodebeemde_plot_bovenmaaiveld25cm_mv2, message=FALSE, warning=FALSE}
temp <- komgr_doodebeemde_tijdreeks %>% 
                  filter(mMaaiveld > 0.25 & as.numeric(str_sub(loc_code,5,7)) > 16) %>% 
                  select(loc_code) %>% 
                  distinct() %>% 
...
in mTAW, voor peilbuizen tot DYLP016, met vermelding van maaiveldhoogte:
```{r tijdreeks_doodebeemde_plot_bovenmaaiveld25cm_taw1, message=FALSE, warning=FALSE}
temp <- komgr_doodebeemde_tijdreeks %>% 
                  filter(mMaaiveld > 0.25 & as.numeric(str_sub(loc_code,5,7)) <= 16) %>% 
                  select(loc_code) %>% 
                  distinct() %>% 
...
in mTAW, voor peilbuizen vanaf DYLP016, met vermelding van maaiveldhoogte:
```{r tijdreeks_doodebeemde_plot_bovenmaaiveld25cm_taw2, message=FALSE, warning=FALSE}
temp <- komgr_doodebeemde_tijdreeks %>% 
                  filter(mMaaiveld > 0.25 & as.numeric(str_sub(loc_code,5,7)) > 16) %>% 
                  select(loc_code) %>% 
                  distinct() %>% 
...
**Samengevat**: raadzaam om enkel de ondiepe peilbuizen te gebruiken (< 2.5 m)
