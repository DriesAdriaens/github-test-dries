...
Vraag of de dataset enkel de (normale, gevalideerde) metingen boven maaiveld moet bevatten, dan wel alle beschikbare metingen, voor de geselecteerde meetpunten. En hoe afwijkende sondemetingen er nog uitgefilterd kunnen worden.

```{r eval=FALSE}
dataset_kal_overstr_dijle <- 
  get_locs(watina, loc_vec = meetpunten_sel %>% 
             # filter(gr_mv == 1) %>% 
             select(loc_code) %>% 
             pull(),
