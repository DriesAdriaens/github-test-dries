...
```{r meetpunten met peilen boven maaiveld langerode}
...
```{r}
meetpunten_sel <- rbind(meetpunten_sel, komgr_langerode_loc %>% filter(filterdepth <= 2.5) %>% collect())
```
...
* overal enkel peilmetingen overhouden die gevalideerd zijn én waaraan geen peilmetingcategorie werd toegekend of enkel de categorieën "peilbuis onder water" en "maaiveld onder water" (`PeilmetingStatusCode == "VLD") & (is.na(PeilmetingCategorie) | PeilmetingCategorieCode %in% c("SOUWA", "TBWWL")`). Hierdoor zullen rariteiten in de manuele metingen alvast verdwijnen.
