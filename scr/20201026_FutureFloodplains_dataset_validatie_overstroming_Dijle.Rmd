...
komgr_doodebeemde_raai1_locs %>% 
  ...
  inner_join(tbl(watina, "DimTijd"), by = c("TijdWID" = "DatumWID")) %>% 
  ggplot() +
  geom_line(data = . %>% filter(grepl("P", loc_code) & loc_code %in% temp),
            aes(x = date(Datum),
                y = mTAW,
                colour = paste0(loc_code, " (", round(soilsurf_ost,2), " mTAW)"))
            , alpha = 1) +
  ...
  geom_line(data = . %>% filter(grepl("S", loc_code)),
            aes(x = date(Datum),
                y = mTAW),
            colour = "black", alpha = 0.75, linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        panel.grid.minor.x = element_line(linetype = "dashed"),
        legend.position = "right") +
  labs(x = "Datum", y = "mTAW", colour = NULL)
...
De komgrond van Egenhoven ligt op de linkeroever van de Dijle en wordt begrensd door de E40 autosnelweg aan de zuidrand en de oude tramdijk die de oostrand vormt. Die tramdijk doorkruist de vallei van noord naar zuid en wordt in het midden doorsneden door de Dijle. Daar bevindt zich ook het kunstwerk waarmee het peil van de Dijle kan geregeld worden en waarmee de bovenstroomse buffercapaciteit van het stroomopwaartse valleigedeelte kan aangesproken worden bij hoge debieten. Hiermee kan het risico op overstromingen stroomafwaarts, ter hoogte van de stad Leuven, verminderd worden.
Het noordelijke deel van het huidige Egenhovenbos bestond eind 18de eeuw uit een complex van visvijvers dat bij het Jezuïetenklooster hoorde.
Deze komgrond hangt topografisch  samen met het deel van de vallei net ten zuiden de E40, eveneens op de linkeroever van de Dijle. Een beek die onder de autosnelweg door stroomt, verbindt beide delen en ontwatert het zuidelijk deel.
...
Op basis van bovenstaand kaartje kunnen we de volgende peilbuiskoppels onderscheiden:
...
Geen overschrijding van maaiveld. Groter stijghoogteverschil (tot 75 cm!) bij lagere stijghoogten (drogere perioden).
...
Geen overschrijding van maaiveld. Het stijghoogteverschil is het grootst (tot 75 cm!) bij lagere stijghoogten, dus in de drogere perioden van het jaar. Het verschil is dan weer te verwaarlozen in de natste perioden van het jaar.
