...
get_stations(variable_name = "waterstand") %>%
  filter(grepl("Langerode", station_name) | grepl("Leigracht", station_name)) %>%
  arrange(station_no) %>%
  knitr::kable()
...
```{r message=FALSE}
get_variables("L08_11A") %>%
  filter(parametertype_name == "H") %>%
  knitr::kable()
```
Omdat de grondwatermetingen ook een tijdsresolutie van 1 dag hebben (indien sondemetingen althans), en de maximale waarde wellicht het meest bepalend is voor de grondwaterstanden, kiezen we hier voor "DagMax". Dat levert voor meetstation K08_001 `ts_id = 24783042` op. Op basis van deze id's vragen we de tijdreeksen op tussen 2000 en 2020.

Preview dataset:

```{r}
L08_11A_timeseries <- get_timeseries_tsid("4082042", from = "2000-01-01", to = "2020-12-01")
```
Tijdreeks:

```{r message=FALSE}
ggplot() +
  geom_line(data = get_timeseries_tsid("4083042", from = "2000-01-01", to = "2020-12-01"), aes(date(ymd_hms(Timestamp)), Value, colour = "L08_11A_min"), alpha = 0.5) +
  geom_line(data = get_timeseries_tsid("4082042", from = "2000-01-01", to = "2020-12-01"), aes(date(ymd_hms(Timestamp)), Value, colour = "L08_11A_max"), alpha = 0.5) +
