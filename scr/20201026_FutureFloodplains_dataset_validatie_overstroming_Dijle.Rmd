...
```{r}
meetpunten_sel <- rbind(meetpunten_sel, komgr_egenhoven_loc %>% filter(filterdepth <= 2.5) %>% collect())
```

# Finale dataset met selectie meetpunten en metingen

Enkel de metingen boven maaiveld, of alle beschikbare metingen voor de geselecteerde meetpunten?

```{r}
peilmetingen_sel_Dijle <- 
  get_locs(watina,
           loc_vec = meetpunten_sel %>% select(loc_code) %>% pull(),
           loc_type = c("P", "B"),
           loc_validity = c("VLD", "ENT"),
           filterdepth_guess = TRUE,
           filterdepth_na = TRUE,
