...
Onderstaand document verkent welke gegevens eventueel in aanmerking komen als dataset ter validatie van de overstromingskaarten.
...
We bekijken dit telkens per komgrond.
...
leaflet() %>%
  ...
  addCircleMarkers(
    data = komgr_neerijse_loc_sf %>% filter(filterdepth < 2.5),
  ...
  addCircleMarkers(
    ...
    group = "Meetpunten >= 2.5 m") %>% 
  addCircleMarkers(
    data = komgr_neerijse_loc_sf %>% filter(is.na(filterdepth)),
...
```{r tijdreeks_plot_neerijse, message=FALSE, warning=FALSE}
...
* oudere metingen gebeurden veelal manueel; pas later werden de automatische meetsondes geïntroduceerd. Die laatste hebben het voordeel dat ze elke dag 2 metingen registreren en daardoor beter in staat zijn tijdelijke overstromingen te detecteren dan met manuele metingen. Die laatste gebeuren in het beste geval slechts om de 14 dagen
...
```{r meetpunten met peilen boven maaiveld neerijse}
komgr_neerijse_tijdreeks %>% 
  filter(mMaaiveld >= 0) %>% 
  select(loc_code) %>% 
  distinct() %>%
...
Het gaat dan nog steeds om `r aantal` van de `r length(komgr_neerijse_loc_vec)` meetpunten in de komgrond van Neerijse. In veel van de meetpunten stegen de stijghoogten dus minstens één of meerdere keren boven het maaiveld uit gedurende de meetperiode.

### Peilmeting met labels die wijzen op effectieve overstromingen

Aan sommige *manuele* metingen (handmetingen dan wel kalibratiemetingen) werd expliciet een bepaald label (peilmetingcategorie) toegekend dat aangeeft of het "maaiveld onder water" stond (code "SOUWA"), dan wel dat de "peilbuis onder water stond" (code "TBWWL"). Tijdens deze metingen werd dus op het terrein met zekerheid vastgesteld dat het maaiveld/de peilbuis overstroomd werd.
...
De betreffende metingen lijken sterk geclusterd in de tijd. Wat meer ingezoomd op de periode met deze metingen levert dat onderstaande figuur op, met opsplitsing naar de twee verschillende peilmetingcategorieën en opgave van de filterdiepte van elke peilbuis.

```{r maaiveld overstroomd tijdreeks ingezoomd neerijse}
temp <- komgr_neerijse_tijdreeks %>% 
                  filter(PeilmetingCategorieCode %in% c("SOUWA", "TBWWL")) %>% 
                  select(loc_code) %>% 
                  distinct() %>% 
...
* Het is opmerkelijk dat heel wat van de peilmetingen met categorie "maaiveld onder water" toch grondwaterpeilen aangeven ónder maaiveld (tot > 5 cm soms) terwijl je hier toch net peilen bóven maaiveld zou verwachten. Mogelijks wijst dat erop dat het overstromingswater van elders afkomstig is (oppervlaktewater dus), en de peilen van oppervlakte- en grondwater -alvast tijdelijk- niet altijd overeenstemmen? En misschien speelt hier ook de nauwkeurigheid een rol (van peilmeting zelf en herrekening naar peil t.o.v. maaiveld)?
* De periode met beide peilmetingcategorieën is uitgesproken kort, net zoals het aantal meetpunten met dergelijke metingen. Wellicht worden deze categorieën niet systematisch genoteerd en toegekend aan handmatige meting in de databank. De lagere frequentie van handmatige metingen doet ook de kans dalen dat op het moment van opmeting het maaiveld net overstroomd wordt, zeker als de overstromingen eerder kortstondig van aard zijn
* DYLP047 en DYLP099 zijn resp. een diepe en ondiepe peilbuis op eenzelfde locatie. Het verschil in stijghoogte loopt in de winterperiode op tot meer dan 25 cm. Die vaststelling wijst alvast op een aanzienlijke kweldruk. De diepere peilbuis (DYLP047) geeft dus alvast niet de stand van de werkelijke watertafel weer en de peilen boven maaiveld wijzen dus zeker niet altijd op een overstroming. De metingen uit de tijdreeks van de ondiepe peilbuis van meetpunt DYLP099 zijn wellicht beter geschikt om dergelijke events aan te tonen. Helaas werd voor beide meetpunten enkel aan de allerlaatste meetwaarden (kalibratiemetingen?) de categorie "maaiveld onder water" toegekend (2019; zie onderstaande tabel) ...
...
Van eventuele peilbuiskoppels selecteren we dus best de peilbuis met kleinste filterdiepte om peilen te selecteren die kunnen wijzen op een effectieve overstroming van het maaiveld. Uiteraard is de absolute filterdiepte ook van belang (zie verder).
...
Uit bovenstaande figuur kunnen we dan kiezen om alle filterdiepten groter dan 3 m als "diep" te beschouwen. Het gaat dan om deze peilbuizen:
...
Hier bekijken we waar in de komgrond er tijdreeksen beschikbaar zijn van oppervlaktewatermetingen, zodat we de samenhang tussen grondwaterstanden en oppervlaktewaterstanden kunnen bekijken, en meer specifiek dan het gelijktijdig optreden van pieken in beide typen waterstanden.
...
Tijdreeks Leigracht in het grijs:

```{r, warning = FALSE}
get_locs(watina, loc_vec = c("DYLP085", "DYLP087","DYLP090", "DYLS102", "DYLS002"), loc_type = c("P", "B", "S")) %>% 
  inner_join(tbl(watina, "FactPeilMeting"),
             by = c("loc_wid" = "MeetpuntWID")) %>% 
  filter(PeilmetingStatusCode == "VLD") %>% 
  inner_join(tbl(watina, "DimTijd"), by = c("TijdWID" = "DatumWID")) %>% 
  ggplot() +
  geom_line(data = . %>% filter(grepl("P", loc_code)),
            aes(x = date(Datum),
                y = mTAW,
  ...
  geom_line(data = . %>% filter(grepl("P", loc_code)),
            aes(x = date(Datum),
                y = soilsurf_ost,
                colour = paste0(loc_code, " (", round(soilsurf_ost,2), " mTAW)"))) +
  geom_line(data = . %>% filter(grepl("S", loc_code)),
            aes(x = date(Datum),
                y = mTAW),
...
* de pieken in de stijghoogten vallen samen met pieken in de oppervlaktewaterstanden van de Leigracht; vooral zichtbaar vanaf de start van de hoogfrequente metingen van de peilen van de Leigracht (DYLS102); dat wijst er op dat er overstromingen plaatsvinden via het oppervlaktewater vanuit de Leigracht (na opstuwing door hoge waterpeilen in de Dijle vanwege aantakking ter hoogte van IJsemonding); de vraag is of de gemeten stijghoogten op het moment van die overstromingen overeenstemmen met de waterpeilen van de overstromingen ...

Focus op metingen boven maaiveld:
...
get_locs(watina, loc_vec = c("DYLP085", "DYLP087","DYLP090", "DYLS102", "DYLS002"), loc_type = c("P", "B", "S")) %>% 
  ...
  inner_join(tbl(watina, "DimTijd"), by = c("TijdWID" = "DatumWID")) %>%
  ggplot() +
  geom_line(data = . %>% filter(grepl("P", loc_code) & mMaaiveld > 0),
            aes(x = date(Datum),
                y = mTAW,
                colour = paste0(loc_code, " (", round(soilsurf_ost,2), " mTAW)"))
            , alpha = 1) +
...
Als we ermee rekening houden dat de peilmetingen van de Leigracht een eind stroomafwaarts gebeuren (DYLS002 en DYLS102), en dus in absolute hoogte wat lager liggen dan de peilen van de Leigracht ter hoogte van de betreffende grondwatermeetpunten (zie figuur), dan is het zeer waarschijnlijk dat de stijghoogten overeenstemmen met de waterpeilen van de Leigracht tijdens overstromingen (die hier dus vanuit de Leigracht optreden).
...
komgr_neerijse_raai2_locs %>% 
  ...
  scale_x_date(date_breaks = "year", date_labels = "%Y") +
  ## clipping zoom
  # xlim(ymd("2010-01-01"), ymd("2011-01-01")) +
  # ylim(-1, 0.1) +
  geom_hline(yintercept = 0, colour = "black", size = 1, linetype = "dotted") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        panel.grid.minor.x = element_line(linetype = "dashed"),
...
Ook hier lijken de stijghoogten overeen te stemmen met de peilen in de Leigracht bij momenten van overstroming (stijghoogten boven maaiveld dus).
...
DYLP072 lijkt in een lokale depressie te liggen.

## Finale selectie grondwatermeetpunten en -metingen

Uit bovenstaande screening besluiten we dat we enkel de meetpunten selecteren met een filterdiepte van minder dan 3 m.
De peilmetingen zelf beperken we tot de gevalideerde metingen (PeilmetingStatus) zonder speciale metingen (PeilmetingCategorie leeg of duidend op overstroming).
...
leaflet() %>%
  ...
  addCircleMarkers(
    ...
    group = "Meetpunten >= 2.5 m") %>% 
  addCircleMarkers(
    data = komgr_doodebeemde_loc_sf %>% filter(is.na(filterdepth)),
    label = ~loc_code, fill = FALSE, weight = 3, color = "orange", opacity = 0.5,
    labelOptions = labelOptions(noHide = TRUE, textOnly = FALSE,
                                direction = "right", opacity = 0.5),
    group = "Meetpunten diepte ongekend") %>%
...
* DYLP008-016
* DYLP017-092
* DYLP006-DYLP015 (2 ondiepe peilbuizen)
...
De tijdreeksen voor de meetpunten met bovengenoemde peilmetingcategorieën zitten vervat in onderstaande figuur, met aanduiding van de betreffende metingen (zwarte punten). Het gaat om meetpunten met ondiepe filters (< 2.5 m).
...
Hieronder de betreffende metingen:
...
We bekijken hieronder de tijdreeksen van de reeds vermelde peilbuiskoppels (ondiep-diep) om te kijken of er sprake is van stijghoogteverschillen tussen de diepe en ondiepe peilbuizen binnen een koppel, die dan kan duiden op een kweldruk. Bij een kweldruk is het alvast wenselijk om enkel de stijghoogten van de ondiepe peilbuis te gebruiken om een idee te krijgen van effectieve overstromingshoogten van het maaiveld. De figuren vermelden de filterdiepte.
...
De stijghoogte overschrijdt hier slechts éénmaal het maaiveld. Deze observatie is eerder twijfelachtig, gezien de overeenkomstige meetwaarde in de ondiepere filter. 
...
In de jaren 1990 lijkt er vrijwel geen sprake van een kweldruk. Vanaf 1998 stijgen de gemeten peilen echter regelmatig boven het maaiveld, en lijkt er wel een drukverschil te zijn tussen beide peilbuizen. Vanaf 2006 (nieuwe peilpuntversie met diepere filter?) lijken de verschillen dan weer af te nemen, met vreemde sprongen (bv. 2010-2013) vanwege een onvolledige dataset. Het is raadzaam om sowieso enkel de stijghoogten van de ondiepe peilbuis (DYLP008) te gebruiken in een mogelijke validatieset voor gemodelleerde overstromingsdieptekaarten.
...
Hier worden geen overschrijdingen van het maaiveld vastgesteld, dus niet relevant. Bovendien lijken de verschillen tussen de stijghoogten in beide peilbuizen eerder beperkt tot onbestaande.

Meer toegespitst op de periode met parallelle metingen op beide filterdiepten:
```{r peilbuiskoppel3_bis doodebeemde}
ggplot(komgr_doodebeemde_tijdreeks %>% 
         filter(loc_code %in% c("DYLP017",
                                "DYLP092")) %>% 
         inner_join(tbl(watina, "DimMetingType"), by = "MetingTypeWID")) +
  ...
  ## non-clipping zoom
  coord_cartesian(#ylim = c(-1, 0.1),
                  xlim = c(ymd("1999-01-01"), ymd("2015-01-01"))
                  ) +
  scale_x_date(date_breaks = "year", date_labels = "%Y") +
  ## clipping zoom
  # xlim(ymd("2013-07-01"), ymd("2015-01-01")) +
...
Beide filters werden blijkbaar ook niet altijd synchroon opgemeten.

* DYLP006-DYLP015
```{r}
ggplot(komgr_doodebeemde_tijdreeks %>% 
         filter(loc_code %in% c("DYLP006",
                                "DYLP015")) %>% 
         inner_join(tbl(watina, "DimMetingType"), by = "MetingTypeWID")) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
...
Nauwelijks verschillen in stijghoogte. Het verschil in filterdiepte is dan ook beperkt. Vreemde uitschieter boven maaiveld eind 2010.

Noot: indien meetpunten ten oosten van de spoorweg ook meegenomen worden, dan komen er nog volgende peilbuiskoppels bij:

* DYLP051-050
```{r peilbuiskoppel4 doodebeemde}
ggplot(komgr_doodebeemde_tijdreeks %>% 
...
Hier zijn wel degelijk verschillen in stijghoogte aanwezig. Vlak aan de voet van de steilrand van de vallei is er een vaak sowieso een hogere kweldruk te verwachten, zelfs in ondiepe peilbuizen.


* Peilbuiskoppel DYLP053-052

```{r peilbuiskoppel5 doodebeemde}
ggplot(komgr_doodebeemde_tijdreeks %>% 
...
```{r meetpunten met peilen boven maaiveld doodebeemde2}
...
Figuur met de tijdreeksen voor de subset van ondiepe meetpunten met minstens één of meerdere stijghoogten boven maaiveld in het westelijk deel van de komgrond:
...
Figuur met de tijdreeksen voor de subset van ondiepe meetpunten met minstens één of meerdere stijghoogten boven maaiveld in het oostelijk deel van de komgrond:
...
in mTAW, ondiepe peilbuizen, westelijk deel, meetreeksen met stijghoogten > 25 cm boven maaiveld, met vermelding van maaiveldhoogte:
...
in mTAW, ondiepe peilbuizen, oostelijk deel, meetreeksen met stijghoogten > 25 cm boven maaiveld, met vermelding van maaiveldhoogte:
...
Enkel ondiepe peilfilters:

```{r}
komgr_doodebeemde_raai1_locs <- get_locs(
  watina, 
  loc_vec = c("DYLB056", "DYLP024", "DYLP056", "DYLP008", "DYLP009", "DYLP012", "DYLP013", "DYLP017", "DYLP063", "DYLS003", "DYLP023"),
  loc_type = c("P", "B", "S"),
  filterdepth_range = c(0,2.5))

komgr_doodebeemde_raai1_locs %>% collect() %>% knitr::kable()
```

#### Ligging:

```{r}
leaflet(komgr_doodebeemde_raai1_locs %>% 
          collect() %>% 
          st_as_sf(coords = c("x", "y"), crs = 31370) %>% 
          st_transform(crs = 4326)) %>% 
  addWMSTiles("https://wms.ngi.be/cartoweb/1/service?", layers = "cartoweb_topo",
...
Beperkt tot ondiepe peilfilters:

```{r}
komgr_doodebeemde_raai1_locs %>% 
  inner_join(tbl(watina, "FactPeilMeting"),
             by = c("loc_wid" = "MeetpuntWID")) %>% 
  filter(PeilmetingStatusCode == "VLD") %>% 
  inner_join(tbl(watina, "DimTijd"), by = c("TijdWID" = "DatumWID")) %>% 
...
Beperkt tot meetreeksen van ondiepe filters met metingen boven maaiveld:

```{r}
temp <- komgr_doodebeemde_raai1_locs %>% 
  inner_join(tbl(watina, "FactPeilMeting"),
             by = c("loc_wid" = "MeetpuntWID")) %>% 
  filter(mMaaiveld > 0 & filterdepth <= 2.5) %>% 
  select(loc_code) %>% 
...
komgr_doodebeemde_raai1_locs %>% 
  inner_join(tbl(watina, "FactPeilMeting"),
             by = c("loc_wid" = "MeetpuntWID")) %>% 
  filter(PeilmetingStatusCode == "VLD" & loc_code %in% temp) %>% 
  inner_join(tbl(watina, "DimTijd"), by = c("TijdWID" = "DatumWID")) %>% 
  ggplot() +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
...
Tijdreeks Leibeek in zwarte stippellijn, bovenop de tijdreeksen in de ondiepe filters met stijghoogten boven maaiveld:
...
ggplot(komgr_doodebeemde_raai1_locs %>% 
         inner_join(tbl(watina, "FactPeilMeting"),
                    by = c("loc_wid" = "MeetpuntWID")) %>% 
         filter(PeilmetingStatusCode == "VLD" &
                  loc_typecode %in% c("P", "B") &
                  loc_code %in% temp) %>% 
         inner_join(tbl(watina, "DimTijd"), by = c("TijdWID" = "DatumWID"))) +
  geom_line(aes(x = date(Datum),
                y = mTAW,
  ...
  # xlim(ymd("2013-07-01"), ymd("2015-01-01")) +
  # ylim(-1, 0.1) +
  geom_line(data = komgr_doodebeemde_raai1_locs %>% 
              inner_join(tbl(watina, "FactPeilMeting"),
                         by = c("loc_wid" = "MeetpuntWID")) %>% 
              filter(PeilmetingStatusCode == "VLD" &
                       loc_typecode == "S") %>% 
...
Het lijkt alvast dat de peilen van de Leibeek nauw samenhangen met veel van de stijghoogten in de peilbuizen op het moment van zowel hoge als lage beekpeilen. In die optiek lijken alvast voor de peilbuizen met ondiepe filter de metingen boven maaiveld indicatief voor de effectieve waterstanden op het moment van overstromingen in de komgrond van de Doode Beemde. 

## Finale selectie grondwatermeetpunten en -metingen

Uit bovenstaande screening besluiten we dat we enkel de meetpunten selecteren met een filterdiepte van minder dan 2.5 m.
De peilmetingen zelf beperken we tot de gevalideerde metingen (PeilmetingStatus) zonder speciale metingen (dus PeilmetingCategorie leeg of enkel duidend op overstroming).
De vraag is of hier nog individuele ondiepe peilbuizen uitgesloten moeten worden omwille van hoge kweldrukken, vnl. dan aan de oostelijke steilrand van de vallei.
...
```{r tijdreeks_langerode_plot2, message=FALSE, warning=FALSE}
ggplot(komgr_langerode_tijdreeks) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
                colour = loc_code)) +
...
* de duur van de tijdreeksen varieert uiteraard per meetpunt. Niet alle meetpunten worden/werden even lang bemeten
* oudere metingen gebeurden veelal manueel; pas later werden de automatische meetsondes geïntroduceerd. Automatische peilsondes hebben het voordeel dat ze elke dag 2 metingen registreren en daardoor beter in staat zijn tijdelijke overstromingen te detecteren dan manuele metingen, die in het beste geval slechts om de 14 dagen gebeuren
...
### Kweldruk en het verschil tussen stijghoogte en waterpeil
Aangezien er geen peilbuiskoppels aanwezig zijn in deze komgrond, is het ook niet mogelijk om een kweldruk aan te tonen op basis van meetgegevens. Tussen de meetpunten DYLP222 en DYLP223, op een afstand van ongeveer 50 m van elkaar gelegen, zit er wel een stijghoogteverschil van ongeveer 20 cm. DYLP223 heeft een filter die ongeveer 80 cm lager zit dan die van DYLP222, maar DYLP223 ligt ook  topografisch wel ongeveer een meter lager in de vallei. Normaal zou je een hogere kweldruk verwachten in de punten meer aan de rand van de vallei.
...
```{r meetpunten met peilen boven maaiveld langerode, message=FALSE}
...
```{r tijdreeks_langerode_west_plot_bovenmaaiveld2, message=FALSE, warning=FALSE}
ggplot(komgr_langerode_tijdreeks %>% 
         filter(loc_code %in% komgr_langerode_loc_fltrd_vec)) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
...
## Oppervlaktewatermeetpunten
Het is onduidelijk of en hoe het deel van de Leigracht in de komgrond van Langerode in contact staat met de Dijle net stroomopwaarts de brug in Korbeek-Dijle. Er zijn geen peilmetingen op de Leigracht zelf. DYLS013 bevat louter enkele eenmalige metingen in oktober 2002.

Ter info kunnen we de meetreeks van de Dijle ter hoogte van de brug van Korbeek-Dijle weergeven, en kijken of de piekwaterstanden indicatief zijn voor de momenten waarop de stijghoogten boven maaiveld komen in de grondwatermeetpunten.

Zoeken van het betreffende meetstation op waterinfo.be op basis van "Korbeek-Dijle":

```{r}
# supported_variables("nl")

get_stations(variable_name = "waterstand") %>%
  filter(grepl("Korbeek-Dijle", station_name)) %>%
  arrange(station_no) %>%
  knitr::kable()

```

Verifiëren van de ligging van het betreffende meetstation (L08_097):

```{r kaart meetstations langerode}
...
get_variables("L08_097") %>%
  ...
  knitr::kable()
```
Omdat de grondwatermetingen ook een tijdsresolutie van 1 dag hebben (indien sondemetingen althans), en de maximale waarde wellicht het meest bepalend is voor de grondwaterstanden, kiezen we hier voor "DagMin" en "DagMax". Dat levert voor meetstation L08_097 resp. `ts_id = 3841042` en `ts_id = 3840042` op. Op basis van deze id's vragen we de tijdreeksen op tussen 2000 en 2020.

Tijdreeks:

```{r message=FALSE}
ggplot() +
  geom_line(data = get_timeseries_tsid("3841042", from = "2000-01-01", to = "2020-12-01"),
            aes(date(ymd_hms(Timestamp)), Value, colour = "L08_097_dagmin"), alpha = 0.5) +
  geom_line(data = get_timeseries_tsid("3840042", from = "2000-01-01", to = "2020-12-01"),
            aes(date(ymd_hms(Timestamp)), Value, colour = "L08_097_dagmax"), alpha = 0.5) +
  ...
  labs(colour = NULL, x = "Datum", y = "Waterstand Dijle (mTAW)")
```
Dagmaxima van de Dijle ter hoogte van Korbeek-Dijle, samen met de tijdreeksen van de grondwatermeetpunten met stijghoogten die boven maaiveld uit komen in de komgrond van Langerode.
```{r}
temp <- komgr_langerode_tijdreeks %>%
  filter(mMaaiveld > 0
         # & filterdepth <= 2.5
         ) %>%
select(loc_code) %>%
...
ggplot(komgr_langerode_tijdreeks %>%
  ...
  geom_line(data = get_timeseries_tsid("3840042", from = "1993-01-01", to = "2020-12-01"),
            aes(date(ymd_hms(Timestamp)), Value, linetype = "Dijle L08_097_dagmax"), alpha = 0.25) +
...
Het lijkt er niet op de er een uitgesproken relatie is tussen de peilen van de Dijle enerzijds en de stijghoogten boven maaiveld in de grondwatermeetpunten in de komgrond van Langerode. Stijghoogten boven maaiveld zijn schaars in deze komgrond. Recente overstromingen lijken zich ook te beperken tot een smalle band (ong. 100 m) rondom de Leigracht zelf (cfr. recent overstroomde gebieden in onderstaand kaartje).
...
leaflet() %>%
  ...
  addLayersControl(
    baseGroups = c("OSM", "Ortho_VL_winter", "Cartoweb"),
    overlayGroups = c("Overstromingsgevoelig", "Recent overstroomd", "Meetpunten < 2.5 m", "Meetpunten >= 2.5 m", "Meetpunten diepte ongekend"),
    options = layersControlOptions(collapsed = FALSE)
  )
```
...
Uit bovenstaande screening besluiten we dat we enkel de meetpunten selecteren met een filterdiepte van minder dan 3 m.
De peilmetingen zelf beperken we tot de gevalideerde metingen (PeilmetingStatus) zonder speciale metingen (dus PeilmetingCategorie leeg of duidend op overstroming).

# Komgrond Leibeek West (tot E40)

Deze komgrond ligt op de linkeroever van de Dijle, tussen de Stationstraat in Korbeek-Dijle in het zuiden en de E40 snelweg in het noorden. De depressies rond Veeweide en Ormendaal worden ontwatert door de Leibeek die centraal in de komgrond in de Dijle uitmondt. De depressie rond Rotspoel (E40) ontwatert naar de komgrond van Egenhoven ten noorden van de E40 via een waterloop (Langestaart) die onder de E40 door stroomt (via pompstation?). Eigenlijk hoort het noordelijke stuk van deze komgrond dus eerder tot de komgrond van Egenhoven, maar de E40 zorgt wel voor een zekere barrière.
...
Voor net iets meer dan de helft van de (historische) peilbuizen is de peilbuislengte, en dus ook de filterdiepte, gekend. In 4 van de 7 peilbuizen met gekende filterdiepte, zit de filter meer dan 2.5 m onder maaiveld. De meetpunten DYLP177 en DYLP277 vormen een peilbuiskoppel, hoewel de filterdiepten slechts ongeveer 1 m verschillen. Beide peilbuizen hebben wel een filterdiepte van meer dan 5 m onder maaiveld.
...
* DYLP200 vertoont geen globaal stijgende trend met onduidelijke seizoenale schommelingen, in tegenstelling tot de tijdreeksen van sommige andere meetpunten; de tijdreeks vertoont daarentegen duidelijke seizoenale schommelingen met buiten de zomerperiode bovendien stijghoogten die quasi continu tot boven het maaiveld reiken; de filterdiepte (2.45 m) doet alvast niet vermoeden dat hier sprake is van een kweldruk

**Samengevat**: de meetreeksen met stijghoogten boven maaiveld lijken weinig bruikbaar voor kalibratie van overstromingsdiepten in deze komgrond.
...
Problemen/pijnpunten bij de meetpunten in de komgrond van Leibeek West:

* veel peilbuizen in deze komgrond uitgerust zijn met diepe filters (>2.5 m onder maaiveld) waardoor een mogelijke kweldruk aanleiding kan geven tot een overschatting van de effectieve grondwatertafel. Door gebrek aan peilbuiskoppels waarin de stijghoogten in peilbuizen met zowel een diepe als een ondiepe filter worden opgemeten, is het moeilijk om een kweldruk uit te sluiten. Het vermoeden is echter dat de stijghoogten van de huidige diepere peilbuizen niet zondermeer gebruik kunnen worden als indicatie voor overstromingen ter hoogte van het maaiveld
...
Uit bovenstaande screening besluiten we dat er in de komgrond van Leibeek West geen meetpunten zijn die metingen bevatten die bruikbaar zijn om gemodelleerde overstromingsdiepten te kalibreren.
...
```{r tijdreeks_leibeekO_plotZ2, message=FALSE, warning=FALSE}
temp <- komgr_leibeekO_tijdreeks %>% 
  filter(mMaaiveld > 0 & loc_code %in% komgr_leibeekO_Z_loc_vec) %>% 
  select(loc_code) %>% 
  distinct() %>% 
...
```{r tijdreeks_leibeekO_plotN, message=FALSE, warning=FALSE}
temp <- komgr_leibeekO_tijdreeks %>% 
...
```{r tijdreeks_leibeekO_plotN2, message=FALSE, warning=FALSE}
temp <- komgr_leibeekO_tijdreeks %>% 
  filter(mMaaiveld > 0 & loc_code %in% komgr_leibeekO_N_loc_vec) %>% 
  select(loc_code) %>% 
  distinct() %>% 
...
* vermoedelijk sterke kweldruk in een aantal meetpunten:
    ...
    * DYLP171: vanwege zeer diepe filter; nabij laagste punt komgrond, langs Leibeek
    * DYLP113 en DYLP116: waarom zijn de peilbuislengten (en dus filterdiepten) niet gedefinieerd; het gaat hier toch om mooie tijdreeksen die bovendien actueel nog bemeten worden; vraag waarom de stijghoogten, die tot 2018 zeer frequent boven maaiveld liggen (kweldruk?), zeer drastisch (tot 1 m) gedaald zijn sinds 2019-2020 ...
* sterke veranderingen in stijghoogten tussen verschillende meetperioden (na onderbreking van metingen):
    ...
    * DYLP224 (langs Leibeek, nabij DYLP121): grote sprong met plots veel lagere stijghoogten vanaf 2013?
...
### Kweldrukken
Er zijn geen peilbuiskoppels aanwezig in deze komgrond, op basis waarvan een kweldruk aangetoond kan worden en zo een verdere selectie kan plaatsvinden van meetpunten/peilbuizen waarvan de tijdreeksen in aanmerking komen om gemodelleerde overstromingen/overstromingsdiepten te kalibreren.

### Stijghoogten boven maaiveld
Zie Tijdreeksen van grondwatermeetpunten; reeds beperkt tot reeksen met stijghoogten boven maaiveld.

## Oppervlaktewatermeetpunten

Onderstaande tabel geeft een overzicht van de peilschalen die aanwezig zijn in de komgrond van de Leibeek Oost en waarvan de meetreeksen in de WATINA databank zitten.

```{r}
komgr_leibeekO_loc_S <- get_locs(watina,
         loc_vec = komgr_leibeekO_loc_vec %>% select(loc_code) %>% pull(),
         loc_type = c("S"),
         loc_validity = c("VLD", "ENT"),
         filterdepth_guess = TRUE,
         ...
         filterdepth_range = c(0, 30)) %>% 
  collect() %>%
  inner_join(komgr_leibeekO_loc_vec, by = "loc_code") %>% 
  st_as_sf(coords = c("x", "y"), crs = 31370) %>% 
  st_transform(crs = 4326)
komgr_leibeekO_loc_S %>% knitr::kable()
```

Ligging van de peilschalen in de komgrond Leibeek Oost:

```{r kaart komgrond leibeek O peilschalen}
pal <- colorFactor(palette = c("red", "blue"), domain = komgr_leibeekO_loc_vec$NZ)
leaflet(komgr_leibeekO_loc_S) %>%
  addWMSTiles("https://wms.ngi.be/cartoweb/1/service?", layers = "cartoweb_topo",
              options = WMSTileOptions(format = "image/png", transparent = TRUE),
              group = "Cartoweb") %>% 
  addTiles(group = "OSM") %>%
  addWMSTiles("https://geoservices.informatievlaanderen.be/raadpleegdiensten/omwrgbmrvl/wms?",
              ...
              group = "Ortho_VL_winter") %>% 
  addCircleMarkers(
    label = ~loc_code, fill = FALSE, weight = 3, color = ~pal(NZ), opacity = 0.5,
    labelOptions = labelOptions(noHide = TRUE, textOnly = FALSE,
                                direction = "right", opacity = 0.5),
    group = "Peilschalen") %>%
  addLegend(position = "bottomleft", title = "Noord/Zuid",
            pal = pal, values = ~NZ, opacity = 1) %>%
  addMiniMap(toggleDisplay = TRUE, zoomLevelOffset = -5) %>% 
  addLayersControl(
    baseGroups = c("OSM", "Ortho_VL_winter", "Cartoweb"),
    overlayGroups = c("Peilschalen"),
    options = layersControlOptions(collapsed = FALSE)
  )
```

Alle peilschalen in deze komgrond staan opgesteld in de Leibeek, behalve DYLS014 (onderdoorgang Stationstraat).

Tijdreeksen (incl. niet gevalideerde metingen voor DYLS010!):

```{r}
komgr_leibeekO_tijdreeks_S <-
  komgr_leibeekO_loc_S <- get_locs(watina,
         loc_vec = komgr_leibeekO_loc_vec %>% select(loc_code) %>% pull(),
         loc_type = c("S"),
         loc_validity = c("VLD", "ENT"),
...
```{r}
ggplot(komgr_leibeekO_tijdreeks_S) +
  geom_line(aes(x = date(Datum),
                y = mTAW,
                colour = loc_code)) +
  ## non-clipping zoom
...
De gegevens zijn voor de meeste meetpunten eerder fragmentarisch omdat ze slecht kortstondig werden opgevolgd. Alle meetwaarden zijn gevalideerd, behalve voor meetpunt DYLS010.

Ter informatie voegen we ook de waterstanden toe van de Dijle ter hoogte van de Stationstraat in Korbeek-Dijle (meetstation K08_097 op waterinfo.be).
...
ggplot(komgr_leibeekO_tijdreeks_S) +
  ...
  #                y = mMaaiveld,
  #                colour = paste0(loc_code, " (", round(mvTAW,2), " mTAW)"))) +
  geom_line(data = K08_097_timeseries, aes(x = date(ymd_hms(Timestamp)),
                                           y = Value, linetype = station_no),
            alpha = 0.25) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        panel.grid.minor.x = element_line(linetype = "dashed"),
        legend.position = "bottom") +
...
We zoomen in op het deel van de tijdreeksen vanaf 2013, omdat de handmatige metingen van 2000-2001 moeilijk te matchen zijn met de hoogfrequente automatische sondemetingen vanaf 2013.
...
ggplot(komgr_leibeekO_tijdreeks_S) +
  geom_line(aes(x = date(Datum),
                ...
                colour = loc_code)) +
  ## non-clipping zoom
  coord_cartesian(xlim = c(ymd("2013-01-01"), ymd("2021-01-01"))) +
  scale_x_date(date_breaks = "year", date_labels = "%Y") +
  ## clipping zoom
  # xlim(ymd("2013-07-01"), ymd("2015-01-01")) +
  # ylim(-1, 0.1) +
...
Uit bovenstaande figuren blijkt alvast dat er weinig samenhang is tussen de peilen van de Leibeek en die van de Dijle.

Hieronder bekijken we de meetreeks van de Dijle samen met de grondwaterstijghoogten in de meetpunten met metingen boven maaiveld om te kijken of er interactie te zien is die erop kan wijzen dat de stijghoogten boven maaiveld samengaan met (pieken in) de waterpeilen in de Dijle of Leibeek.

Noordelijk deel, peilen van Dijle in lichtgrijs:
...
ggplot(komgr_leibeekO_tijdreeks %>% 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        panel.grid.minor.x = element_line(linetype = "dashed"),
        legend.position = "right") +
  labs(x = "Datum", y = "mTAW", colour = "Meetpunt", linetype = "filterdiepte")
```
```{r message=FALSE}
temp <- komgr_leibeekO_tijdreeks %>% 
  filter(mMaaiveld > 0 & loc_code %in% komgr_leibeekO_N_loc_vec) %>% 
...
Uit bovenstaande screening besluiten we dat we enkel de meetpunten selecteren met een (gekende) filterdiepte van minder dan 2.5 m.
De peilmetingen zelf beperken we tot de gevalideerde metingen (PeilmetingStatus) zonder speciale metingen (PeilmetingCategorie leeg of duidend op overstroming).
De vraag is dan nog of ook DYLP107 nog moet uitgesloten worden (kweldruk).

# Komgrond Egenhovenbos

De komgrond van Egenhoven ligt op de linkeroever van de Dijle en wordt begrensd door de E40 autosnelweg aan de zuidrand en de oude tramdijk aan de oostrand. Die tramdijk doorkruist de vallei van noord naar zuid en wordt in het midden doorsneden door de Dijle. Daar bevindt zich ook het kunstwerk waarmee het peil van de Dijle kan geregeld worden. Zo kan de bovenstroomse buffercapaciteit van het stroomopwaartse valleigedeelte aangesproken worden bij hoge debieten, waardoor het risico op overstromingen stroomafwaarts, ter hoogte van de stad Leuven, afneemt.
Het noordelijke deel van het huidige Egenhovenbos bestond eind 18de eeuw uit een complex van visvijvers die bij het Jezuïetenklooster hoorden.
De komgrond van het Egenhovenbos hangt topografisch  samen met het deel van de vallei net ten zuiden de E40, eveneens op de linkeroever van de Dijle. Een beek die onder de autosnelweg door stroomt, verbindt beide delen en ontwatert zo ook het zuidelijk deel.
...
Spreiding qua filterdiepte van de peilbuizen, wetende dat voor één van de peilbuizen geen filterdiepte gekend is:
...
Hoewel ze op enkele meters van elkaar gelegen zijn, vormen peilbuizen DYLP161 en DYLP162 een peilbuiskoppel met resp. diepe en ondiepe filterdiepte.
...
Zoals uit de tijdreeksen hierboven al visueel kan opgemaakt worden, gaat het merendeel van de metingen om automatische sondemetingen. De hoge densiteit aan metingen (2/dag) en de vele kleine sprongen op korte tijd (dag/nacht) die zorgen voor een band eerder dan een lijn van punten, wijzen hierop. Aan automatische metingen worden bovenstaande labels evenwel niet toegekend omdat er geen terreinverificatie plaatsvindt op het moment dat de meting geregistreerd wordt (tenzij dan tijdens de sporadische, handmatige kalibratiemetingen). In de komgrond van Egenhoven werden geen manuele metingen geregistreerd met de vermelding dat het maaiveld of de peilbuis onder water stond op het moment van opmeting.

### Kweldrukken

Uit de metadata van de peilbuizen in de komgrond van Egenhovenbos blijkt dat de peilbuizen van de meetpunten DYLP161 en DYLP162 een peilbuiskoppel vormen, met DYLP161 als diepe en DYLP162 als ondiepe peilbuis.
...
get_variables("K08_012") %>% 
  ...
  knitr::kable()
```
Omdat de grondwatermetingen een tijdsresolutie van een halve dag hebben, en de maximale waarde wellicht het meest bepalend is voor de grondwaterstanden, kiezen we hier voor "DagMax" voor beide meetstations. Dat levert voor meetstation K08_011 `ts_id = 24980042` (Zijloop) op, en voor meetstation K08_012 `ts_id = 31429042` (Dijle). Op basis van deze id's vragen we de tijdreeksen op tussen 2000 en 2020.
...
ggplot(K08_timeseries %>% filter(Value > 20)) +
  ...
  labs(colour = NULL)
```
Peilen van de Dijle ter hoogte van meetstation K08_012, samen met de tijdreeksen van de ondiepe grondwatermeetpunten met stijghoogten boven maaiveld:
...
Peilen van de loop Langestaart ter hoogte van meetstation K08_011, samen met de tijdreeksen van de ondiepe grondwatermeetpunten met stijghoogten boven maaiveld:
...
**Samengevat**: de stijghoogten boven maaiveld van peilbuizen met een filterdiepte van minder dan 2.5 m geven wellicht een realistisch beeld van de effectieve watertafel boven maaiveld tijdens overstromingen.
...
selectie van peilbuizen met filterdiepte < 2.5 m (peilbuis met ongekende filterdiepte heeft geen stijghoogten boven maaiveld) én gemeten stijghoogten boven maaiveld.

# Komgrond Leigracht Oost

Het gaat hier om een zone op de rechteroever (oost) van Dijle. De waterloop "Grens van Oud-Heverlee" vormt de noordelijke grens, de Neerijsebaan de zuidelijke, de oeverwallen van de Dijle de westelijke, en in het oosten is er de vrij steile valleiflank. Ook hier doorkruist een diepe gracht de komgrond van zuid naar noord, de Leigracht. Ter hoogte van de Reigerstraat mondt die uit in de Dijle. Het is niet geheel duidelijk of het stukje tussen de Reigerstraat en de waterloop "Grens van Oud-Heverlee" nog tot deze komgrond behoort, dan wel tot die van de Doode Beemde (zie hoger). Hieronder worden de meetpunten in dat deel alvast wel tot deze komgrond gerekend.
...
```{r}
komgr_leigrachtO_loc <- get_locs(watina,
                     loc_vec = c("DYLP011", "DYLP157", "DYLP010", "DYLB058", "DYLP030", "DYLP032", "DYLP044", "DYLP045", "DYLP058", "DYLP060", "DYLP061", "DYLP067", "DYLP068", "DYLP094", "DYLP096", "DYLP154", "DYLP155", "DYLP156", "DYLP003", "DYLP001", "DYLP002", "DYLP057"),
                     loc_type = c("P", "B"),
                     loc_validity = c("VLD", "ENT"),
                     filterdepth_guess = TRUE,
                     filterdepth_na = TRUE,
                     filterdepth_range = c(0, 30))
komgr_leigrachtO_loc %>% collect() %>% knitr::kable()
```

Voor één van de peilbuizen is de peilbuislengte, en dus ook de filterdiepte, niet gekend.

```{r}
komgr_leigrachtO_loc_sf <- komgr_leigrachtO_loc %>%
  collect() %>% 
  st_as_sf(coords = c("x", "y"), crs = 31370) %>% 
  st_transform(crs = 4326)
...
leaflet() %>%
  ...
  addWMSTiles("https://geoservices.informatievlaanderen.be/raadpleegdiensten/omwrgbmrvl/wms?",
              ...
              group = "Ortho_VL_winter") %>% 
  addCircleMarkers(
    data = komgr_leigrachtO_loc_sf %>% filter(filterdepth < 2.5),
    label = ~loc_code, fill = FALSE, weight = 3, color = "red", opacity = 0.5,
    labelOptions = labelOptions(noHide = TRUE, textOnly = FALSE,
                                direction = "right", opacity = 0.5),
    group = "Meetpunten < 2.5 m") %>% 
  addCircleMarkers(
    ...
    group = "Meetpunten >= 2.5 m") %>% 
  addCircleMarkers(
    data = komgr_leigrachtO_loc_sf %>% filter(is.na(filterdepth)),
    label = ~loc_code, fill = FALSE, weight = 3, color = "orange", opacity = 0.5,
    labelOptions = labelOptions(noHide = TRUE, textOnly = FALSE,
                                direction = "right", opacity = 0.5),
    group = "Meetpunten diepte ongekend") %>%
...
Spreiding qua filterdiepte van de peilbuizen, wetende dat voor één peilbuis geen filterdiepte bekend is:

```{r message = FALSE, warning = FALSE, error = FALSE, echo = FALSE}
komgr_leigrachtO_loc %>% 
  ggplot() +
  geom_histogram(aes(x = filterdepth), binwidth = 0.10)
```

Op basis van bovenstaand kaartje kunnen we de volgende peilbuiskoppels onderscheiden in de komgrond van de Leigracht Oost:

* diep (>2.5m) - ondiep (<=2.5m):
    + DYLP045 - DYLP044
    + DYLP032 - DYLP003
    + DYLP068 - DYLP096
* diep (>>2.5m) - diep (>2.5m):
...
Voor de geselecteerde grondwatermeetpunten vragen we de gevalideerde peilmetingen op (`PeilmetingStatusCode == "VLD"`), die bovendien als betrouwbaar gelden (`is.na(PeilmetingCategorie) | PeilmetingCategorieCode %in% c("SOUWA", "TBWWL")`).
```{r tijdreeks leigrachtO}
komgr_leigrachtO_tijdreeks <-
  komgr_leigrachtO_loc %>%
  # filter(filterdepth <= 3) %>% 
  rename(mvTAW = soilsurf_ost) %>% 
  inner_join(tbl(watina, "FactPeilMeting"),
...
Onderstaande figuur geeft de tijdreeksen voor de verschillende meetpunten, ongeacht de filterdiepte.

```{r tijdreeks_leigrachtO_plot, message=FALSE, warning=FALSE}
ggplot(komgr_leigrachtO_tijdreeks) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
                colour = loc_code)) +
  ## non-clipping zoom
...
Dergelijke metingen werden vastgesteld bij één enkel meetpunt:

```{r maaiveld overstroomd leigrachtO}
komgr_leigrachtO_tijdreeks %>% 
  filter(PeilmetingCategorieCode %in% c("SOUWA", "TBWWL")) %>% 
  select(loc_code) %>% 
  distinct() %>%
  inner_join(komgr_leigrachtO_loc, by = "loc_code") %>% 
  knitr::kable()
```

Voor de overige meetpunten met stijghoogten boven maaiveld, is er dus geen absolute zekerheid dat de watertafel effectief boven maaiveld stond op het moment van opmeting, en er dus effectief sprake was van een overstroming, dan wel dat er (tijdelijk) een hogere/verhoogde kweldruk aanwezig was.

De tijdreeksen voor de meetpunten met bovengenoemde peilmetingcategorieën zitten vervat in onderstaande figuur, met aanduiding van de betreffende metingen (zwarte punten).

```{r maaiveld overstroomd tijdreeks leigrachtO}
temp <- komgr_leigrachtO_tijdreeks %>% 
                  filter(PeilmetingCategorieCode %in% c("SOUWA", "TBWWL")) %>% 
                  select(loc_code) %>% 
                  distinct() %>% 
                  pull()
...
ggplot(komgr_leigrachtO_tijdreeks %>% 
  ...
  # xlim(ymd("2013-07-01"), ymd("2015-01-01")) +
  # ylim(-1, 0.1) +
  geom_point(data = komgr_leigrachtO_tijdreeks %>% 
               filter(PeilmetingCategorieCode %in% c("SOUWA", "TBWWL")),
             aes(x = date(Datum),
                 y = mMaaiveld)) +
  geom_hline(yintercept = 0, colour = "black", size = 1, linetype = "dotted") +
...
Concreet gaat het hier om één enkele manuele meting waaraan het label "SOUWA" werd toegekend.
```{r peilmetingen SOUWA TBWWL leigrachtO}
komgr_leigrachtO_tijdreeks %>% 
  filter(PeilmetingCategorieCode %in% c("SOUWA", "TBWWL")) %>% 
  select(loc_code, obswell_state, Datum, mMaaiveld, PeilmetingCategorie, PeilmetingCategorieCode) %>% 
  knitr::kable()
...
We onderscheiden de volgende peilbuiskoppels in de komgrond van de Leigracht Oost:
...
We bekijken hieronder de tijdreeksen van beide peilbuizen per koppel om te kijken of er sprake is van stijghoogteverschillen tussen beide, wat dan kan duiden op een kweldruk. Bij de aanwezigheid van een opwaartse kweldruk is het alvast wenselijk om enkel de stijghoogten van de ondiepe (<2.5-3 m) peilbuis te gebruiken om een idee te krijgen van effectieve overstromingshoogten van het maaiveld. Hoe ondieper de filter van een peilbuis, hoe beter de gemeten stijghoogten immers overeenkomen met de werkelijke grondwatertafel.

De filterdiepten van elk meetpunt staan telkens vermeld in onderstaande figuren. 

* DYLP045 - DYLP044

```{r peilbuiskoppel1 LeigrachtO}
ggplot(komgr_leigrachtO_tijdreeks %>% 
         filter(loc_code %in% c("DYLP045",
                                "DYLP044")) %>% 
         inner_join(tbl(watina, "DimMetingType"), by = "MetingTypeWID")) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
...
Uit bovenstaande figuur blijkt dat de stijghoogten bij diepe filter toch minstens een 15 à 20-tal cm hoger liggen. De stijghoogten bij de diepe filter overschrijden quasi jaarlijks de maaiveldhoogte, terwijl dat bij de ondiepe filter niet het geval is.

* DYLP032 - DYLP003

```{r peilbuiskoppel2 LeigrachtO}
ggplot(komgr_leigrachtO_tijdreeks %>% 
         filter(loc_code %in% c("DYLP032",
                                "DYLP003")) %>% 
         inner_join(tbl(watina, "DimMetingType"), by = "MetingTypeWID")) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
...
Ook hier overschrijden de stijghoogten in de diepere filter quasi jaarlijks de maaiveldhoogte, terwijl die frequentie voor de ondiepe filter veel lager ligt. De stijghoogteverschillen lopen op tot meer dan 20 cm. 

* DYLP068 - DYLP096

```{r peilbuiskoppel3 LeigrachtO}
ggplot(komgr_leigrachtO_tijdreeks %>% 
         filter(loc_code %in% c("DYLP068",
                                "DYLP096")) %>% 
         inner_join(tbl(watina, "DimMetingType"), by = "MetingTypeWID")) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
...
Geen overschrijding van maaiveld. Minder eenduidig, maar het stijghoogteverschil is alleszins minder uitgesproken dan in voorgaande peilbuiskoppels.

* DYLP067 - DYLP094

```{r peilbuiskoppel4 LeigrachtO}
ggplot(komgr_leigrachtO_tijdreeks %>% 
         filter(loc_code %in% c("DYLP067",
                                "DYLP094")) %>% 
         inner_join(tbl(watina, "DimMetingType"), by = "MetingTypeWID")) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
...
Geen overschrijding van maaiveld. Groter stijghoogteverschil (tot 1 m!) bij lagere stijghoogten (drogere perioden).

* DYLP061 - DYLP060

```{r peilbuiskoppel5 LeigrachtO}
ggplot(komgr_leigrachtO_tijdreeks %>% 
         filter(loc_code %in% c("DYLP061",
                                "DYLP060")) %>% 
         inner_join(tbl(watina, "DimMetingType"), by = "MetingTypeWID")) +
  geom_line(aes(x = date(Datum),
                y = mMaaiveld,
...
Geen overschrijding van maaiveld. Het stijghoogteverschil is het grootst (tot 1 m!) bij lagere stijghoogten, dus in de drogere perioden van het jaar. Het verschil is dan weer te verwaarlozen in de natste perioden van het jaar.

**Samengevat**: selectie van peilbuizen met filterdiepte < 2.5 m lijkt aangewezen

### Stijghoogten boven maaiveld

Als we enkel de peilbuizen beschouwen met filterdiepte < 2.5 m én (occasionele) stijghoogten boven maaiveld, dan houden we volgende tijdreeksen over:

```{r tijdreeks_leigrachtO_mv1, message=FALSE, warning=FALSE}
temp <- komgr_leigrachtO_tijdreeks %>% 
  filter(mMaaiveld > 0 & filterdepth <= 2.5) %>%
  select(loc_code) %>% 
  distinct() %>% 
...
```{r tijdreeks_leigrachtO_taw1, message=FALSE, warning=FALSE}
temp <- komgr_leigrachtO_tijdreeks %>% 
  filter(mMaaiveld > 0 & filterdepth <= 2.5) %>%
  select(loc_code) %>% 
  distinct() %>% 
...
**Samengevat**: selectie van peilbuizen met filterdiepte < 2.5 m én gemeten stijghoogten boven maaiveld

## Oppervlaktewatermeetpunten
<!-- Meetstation waterinfo.be: L08_098 -->

Het is onduidelijk of en hoe het deel van de Leigracht in de komgrond van Leigracht Oost in contact staat met de Dijle net stroomopwaarts de brug ter hoogte van de Reigerstraat. Er zijn geen peilmetingen op de Leigracht zelf.

Ter info kunnen we de meetreeks van de Dijle ter hoogte van de brug van Sint-Joris-Weert weergeven, en kijken of de piekwaterstanden indicatief zijn voor de momenten waarop de stijghoogten boven maaiveld komen in de grondwatermeetpunten.

Zoeken van het betreffende meetstation op waterinfo.be op basis van "Sint-Joris-Weert":

```{r}
# supported_variables("nl")

get_stations(variable_name = "waterstand") %>%
  filter(grepl("Sint-Joris-Weert", station_name)) %>%
  arrange(station_no) %>%
  knitr::kable()

```

Verifiëren van de ligging van het betreffende meetstation (L08_098):

```{r kaart meetstations leigrachtO}
...
```{r message=FALSE}
get_variables("L08_098") %>%
  filter(parametertype_name == "H") %>%
  knitr::kable()
```
Omdat de grondwatermetingen ook een tijdsresolutie van een halve dag hebben (indien sondemetingen althans), en de maximale waarde wellicht het meest bepalend is voor de grondwaterstanden, kiezen we hier voor "DagMin" en "DagMax". Dat levert voor meetstation L08_097 resp. `ts_id = 3863042` en `ts_id = 3862042` op. Op basis van deze id's vragen we de tijdreeksen op sinds 1988.

Tijdreeksen:

```{r message=FALSE}
ggplot() +
  geom_line(data = get_timeseries_tsid("3862042", from = "1988-01-01"),
            aes(date(ymd_hms(Timestamp)), Value, colour = "L08_098_dagmax"), alpha = 0.75) +
  geom_line(data = get_timeseries_tsid("3863042", from = "1988-01-01"),
            aes(date(ymd_hms(Timestamp)), Value, colour = "L08_098_dagmin"), alpha = 0.75) +
  # geom_line(data = get_locs(watina, loc_vec = c("DYLS102"), loc_type = c("S")) %>%
  #             inner_join(tbl(watina, "FactPeilMeting"),
  #                        by = c("loc_wid" = "MeetpuntWID")) %>%
  #             filter(PeilmetingStatusCode == "VLD") %>%
...
Dagmaxima van de Dijle ter hoogte van Korbeek-Dijle, samen met de tijdreeksen van de ondiepe grondwatermeetpunten met stijghoogten die boven maaiveld uit komen in de komgrond van Leigracht Oost.
```{r warning=FALSE, message=FALSE, fig.height=10}
temp <- komgr_leigrachtO_tijdreeks %>%
  filter(mMaaiveld > 0
         & filterdepth <= 2.5
...
ggplot(komgr_leigrachtO_tijdreeks %>%
         filter(loc_code %in% temp)) +
  geom_line(data = get_timeseries_tsid("3862042", from = "1988-01-01"),
            aes(date(ymd_hms(Timestamp)), Value, linetype = "Dijle L08_098_dagmax"), alpha = 0.25) +
  geom_line(aes(x = date(Datum),
                y = mTAW,
                # colour = loc_code,
                colour = paste0(loc_code, " (", round(mvTAW,2), " mTAW)")
...
Hieruit blijkt dat er geen sterk uitgesproken relatie is tussen de peilen van de Dijle enerzijds en anderzijds de stijghoogten boven maaiveld in de grondwatermeetpunten in de komgrond van Leigracht Oost. Wel volgen stijghoogten boven maaiveld vaak op één of meerdere opeenvolgende pieken in de waterpeilen van de Dijle.

Recente overstromingen lijken zich  te beperken tot een smalle band (ong. 100 m) rondom de Leigracht zelf (cfr. recent overstroomde gebieden in onderstaand kaartje).
...
leaflet() %>%
  ...
  addWMSTiles("https://inspirepub.waterinfo.be/arcgis/services/Overstromingsgevoelige_gebieden_2017/MapServer/WMSServer?",
              layers = "0",
              options = WMSTileOptions(format = "image/png", transparent = TRUE, opacity = 0.25),
              group = "Overstromingsgevoelig") %>% 
  addWMSTiles("https://geoservices.informatievlaanderen.be/raadpleegdiensten/VMM/wms?",
              layers = "RecOvstrGeb",
              options = WMSTileOptions(format = "image/png", transparent = TRUE, opacity = 0.25),
              group = "Recent overstroomd") %>% 
  hideGroup("Overstromingsgevoelig") %>% 
  addCircleMarkers(
    data = komgr_leigrachtO_loc_sf %>% filter(filterdepth < 2.5),
    label = ~loc_code, fill = FALSE, weight = 3, color = "red", opacity = 0.5,
    labelOptions = labelOptions(noHide = TRUE, textOnly = FALSE,
...
# Finale dataset met selectie meetpunten en metingen

```{r}
meetpunten_sel <- rbind(
  komgr_neerijse_loc %>% filter(filterdepth <= 3) %>% collect(),
  komgr_doodebeemde_loc %>% filter(filterdepth <= 2.5) %>% collect(),
  komgr_langerode_loc %>% filter(filterdepth <= 3) %>% collect(),
  # niets voor leibeekW ...
  komgr_leibeekO_loc %>% filter(filterdepth <= 2.5) %>% collect(),
  komgr_egenhoven_loc %>% filter(filterdepth <= 2.5) %>% collect(),
  komgr_leigrachtO_loc %>% filter(filterdepth <= 2.5) %>% collect()
)
```

```{r}
meetpunten_sel_gr_mv <- 
  get_locs(watina, loc_vec = meetpunten_sel %>% 
             select(loc_code) %>% 
             pull(),
           loc_type = c("P", "B"),
           loc_validity = c("VLD", "ENT"),
  ...
  distinct() %>%
  collect() %>% 
  inner_join(meetpunten_sel, by = "loc_code")
```

```{r message=FALSE}
meetpunten_sel_def <- 
  meetpunten_sel %>% 
  full_join(meetpunten_sel_gr_mv %>% 
              mutate(gr_mv = 1)) %>% 
  mutate(gr_mv = ifelse(is.na(gr_mv), 0, 1))
```

In totaal levert de selectie `r nrow(meetpunten_sel_def)` meetpunten op, waarvan er `r nrow(meetpunten_sel_gr_mv)` zijn met (gevalideerde) stijghoogten boven maaiveld (zie veld "gr_mv").

```{r}
meetpunten_sel_def %>% knitr::kable()
```

Overzichtskaartje hiervan:

```{r message=FALSE}
pal_gr_mv <- colorFactor(palette = c("red", "blue"), domain = meetpunten_sel_def$gr_mv) 

leaflet(meetpunten_sel_def %>% 
          st_as_sf(coords = c("x", "y"), crs = 31370) %>% 
          st_transform(crs = 4326)) %>%
  addWMSTiles("https://wms.ngi.be/cartoweb/1/service?", layers = "cartoweb_topo",
              options = WMSTileOptions(format = "image/png", transparent = TRUE),
  ...
  addWMSTiles("https://geoservices.informatievlaanderen.be/raadpleegdiensten/VMM/wms?",
              ...
              group = "Recent overstroomd") %>% 
  addCircleMarkers(
    data = get_locs(watina, area_codes = "DYL",
                    loc_type = c("P", "B", "S"),
                    filterdepth_guess = TRUE,
                    filterdepth_range = c(0,30),
                    filterdepth_na = TRUE
      ...
      collect() %>%
      anti_join(meetpunten_sel_def) %>%
      filter(!is.na(x) |!is.na(y)) %>% 
      st_as_sf(coords = c("x", "y"), crs = 31370) %>% 
      st_transform(crs = 4326),
    label = ~loc_code, fill = FALSE, weight = 3, color = "grey", opacity = 0.5,
    labelOptions = labelOptions(noHide = FALSE,
                                textOnly = FALSE,
                                direction = "right", opacity = 0.75),
    group = "Meetpunten overig") %>% 
  addCircleMarkers(
    label = ~loc_code,
    fill = FALSE, weight = 3, color = ~pal_gr_mv(gr_mv), opacity = 0.5,
    labelOptions = labelOptions(noHide = FALSE,
                                textOnly = FALSE,
                                direction = "right", opacity = 0.75),
    group = "Meetpunten selectie") %>% 
  # addMiniMap(toggleDisplay = TRUE, zoomLevelOffset = -5) %>% 
  hideGroup(c("Recent overstroomd", "Overstromingsgevoelig", "Meetpunten overig")) %>% 
  addLayersControl(
    baseGroups = c("OSM", "Ortho_VL_winter", "Cartoweb"),
    overlayGroups = c("Overstromingsgevoelig", "Recent overstroomd",
                      "Meetpunten selectie", "Meetpunten overig"),
    options = layersControlOptions(collapsed = FALSE)) %>% 
  addLegend(pal = pal_gr_mv, values = ~gr_mv, title = "metingen boven mv",
            group = "Meetpunten selectie")
```

Vraag of de dataset enkel de (normale, gevalideerde) metingen bóven maaiveld moet bevatten, dan wel alle beschikbare metingen, voor de geselecteerde meetpunten. En hoe afwijkende sondemetingen er nog uitgefilterd kunnen worden.

Dataset met enkel metingen boven maaiveld:
```{r}
WATINA_meas_above_surf_dijle <- 
  get_locs(watina, loc_vec = meetpunten_sel_def %>% 
             filter(gr_mv == 1) %>%
             select(loc_code) %>% 
             pull(),
           loc_type = c("P", "B"),
  ...
  collect()
summary(WATINA_meas_above_surf_dijle)
```
Aantal meetpunten:
```{r}
WATINA_meas_above_surf_dijle %>% 
...
```
```{r figuren tijdreeksen TAW, eval=FALSE, include=FALSE}
 get_locs(watina, loc_vec = meetpunten_sel_def %>% 
           filter(gr_mv == 1) %>%
           select(loc_code) %>% 
           pull(),
...
plyr::d_ply(
    ...
    function(deze.subset){
      p <- ggplot(deze.subset) +
        geom_line(aes(x = date(Datum),
                      y = mTAW,
                      colour = paste0(obswell_code, " (", round(mvTAW,2), " mTAW)"))) +
        scale_x_date(date_breaks = "year", date_labels = "%Y") +
        geom_line(aes(x = date(Datum),
                      y = mvTAW,
                      colour = paste0(obswell_code, " (", round(mvTAW,2), " mTAW)"))) +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
              panel.grid.minor.x = element_line(linetype = "dashed"),
              legend.position = "right") +
        labs(x = "Datum", y = "mTAW", colour = NULL, title = deze.subset$loc_code[1])
...
```{r figuren tijdreeksen mv, eval=FALSE, include=FALSE}
get_locs(watina, loc_vec = meetpunten_sel_def %>% 
           filter(gr_mv == 1) %>%
           select(loc_code) %>% 
           pull(),
  ...
  inner_join(tbl(watina, "FactPeilMeting"),
             by = c("loc_wid" = "MeetpuntWID")) %>% 
  filter(PeilmetingStatusCode == "VLD" & (is.na(PeilmetingCategorie) | 
                                            PeilmetingCategorieCode %in% c("SOUWA", "TBWWL"))) %>%   inner_join(tbl(watina, "DimTijd"), by = c("TijdWID" = "DatumWID")) %>% 
  inner_join(tbl(watina, "DimMetingType"), by = "MetingTypeWID") %>% 
  rename(mvTAW = soilsurf_ost) %>%
  collect() %>% 
  plyr::d_ply(
    ...
    function(deze.subset){
      p <- ggplot(deze.subset) +
        ...
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
              panel.grid.minor.x = element_line(linetype = "dashed"),
              legend.position = "right") +
        labs(x = "Datum", y = "m-mv", title = deze.subset$loc_code[1], colour = NULL, linetype = NULL)
      ggsave(p, filename = paste0(deze.subset$loc_code, "_tijdreeks_mv.jpeg"), device = "jpeg",
             width = 8, height = 5.7, units = "in")
    }
  )
```
...
* DYLP008/016: is dit geen LTER meetstation? Waarom werden de laatste jaren dan nog geen gevalideerde waarnemingen aangevuld in de databank?
...
```{r klad tijdreeks meetpunt mTAW, warning = FALSE}
get_locs(watina,
         loc_vec = c("DYLP005"),
         loc_type = c("P", "B", "S"),
         filterdepth_guess = TRUE,
...
<!-- ```{r klad tijdreeks meetpunt m-mv} -->
<!-- get_locs(watina, -->
...
<!-- ```{r klad tijdreeks meetpunt m-mv 2} -->
<!-- get_locs(watina, -->
...
```{r klad tijdreeks peilpunt mTAW, warning = FALSE}
get_locs(watina,
         loc_vec = c("DYLP005"),
         loc_type = c("P", "B", "S"),
         filterdepth_guess = TRUE,
